...
import config
from TeamDevXMusic import Apple, Resso, SoundCloud, Spotify, Telegram, YouTube, app
from TeamDevXMusic.relay.caller import TeamDevXMusic
from TeamDevXMusic.toolkit import seconds_to_min, time_to_seconds
from TeamDevXMusic.toolkit.chplay import get_channeplayCB
from TeamDevXMusic.toolkit.datastore import is_video_allowed
from TeamDevXMusic.toolkit.guards.language import languageCB
from TeamDevXMusic.toolkit.guards.play import PlayWrapper
from TeamDevXMusic.toolkit.fmt import formats
from TeamDevXMusic.toolkit.panels.play import (
    livestream_markup,
    playlist_markup,
    slider_markup,
    track_markup,
...
from TeamDevXMusic.toolkit.datastore import is_served_user
...
async def play_commnd(
    ...
    PLAY_STICKER_ID = "CAACAgQAAxkBAAEd3QVp-gZ0gxB5QT5jZlX-c94DCbDRLQACvA4AAouy2FLwO8eGWP2PTDsE"
    if channel:
        mystic = await message.reply_text(_["play_2"].format(channel))
    else:
        mystic = await message.reply_sticker(PLAY_STICKER_ID)
    plist_id = None
    slider = None
    plist_type = None
    spotify = None
...
@app.on_callback_query(filters.regex("TeamDevPlaylists") & ~BANNED_USERS)
@languageCB
