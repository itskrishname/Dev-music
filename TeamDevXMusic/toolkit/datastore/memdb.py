...
videodb = mongodb.teamdevvideocalls
...
async def is_autoplay_on(chat_id: int) -> bool:
    ...
    result = bool(doc)
    autoplay[chat_id] = result
    return result


async def autoplay_on(chat_id: int):
    autoplay[chat_id] = True
    exists = await autoplaydb.find_one({"chat_id": chat_id})
    if not exists:
...
def set_autoplay_last(chat_id: int, vidid: str):
    autoplay_last[chat_id] = vidid


def get_autoplay_last(chat_id: int) -> str | None:
    return autoplay_last.get(chat_id)
