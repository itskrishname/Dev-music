...
import config
from TeamDevXMusic import LOGGER, YouTube, app
        ...
        try:
            ...
            if not check:
                ...
                if await is_autoplay_on(chat_id):
                    last_vidid = get_autoplay_last(chat_id)
                    if last_vidid:
                        try:
                            from TeamDevXMusic.toolkit.autoplay import TeamDev_trigger_autoplay
                            await TeamDev_trigger_autoplay(client, chat_id, last_vidid)
                            return
                        except Exception as ap_err:
                            LOGGER("TeamDevXMusic.autoplay").warning(
                                f"AutoPlay failed for {chat_id}: {ap_err}"
                            )
                # ── end AutoPlay hook ──────────────────────────────────
            ...
            # ── Track last YouTube vidid for AutoPlay ──────────────────
            if videoid not in ("telegram", "soundcloud") and "index_" not in queued:
                set_autoplay_last(chat_id, videoid)
            if "live_" in queued:
